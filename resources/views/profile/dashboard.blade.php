...
<div class="py-12 bg-slate-50 min-h-screen"
     x-data="{
         ...
         loadMockData() {
             // Load appointments from localStorage or use mock
             const savedAppts = localStorage.getItem('nks_appointments');
             if (savedAppts) {
                 this.appointments = JSON.parse(savedAppts);
             } else {
                 this.appointments = [
                     {
                         id: 1,
                         property_title: 'Căn hộ dịch vụ Full nội thất ngay trung tâm Quận 1',
                         property_slug: 'can-ho-dich-vu-quan-1',
                         date: '2026-06-08',
                         time: '14:30',
                         status: 'confirmed',
                         host_name: 'Anh Minh',
                         host_phone: '0932030958'
                     }
                 ];
             ...
             if (savedOwnerProps) {
                 this.ownerProperties = JSON.parse(savedOwnerProps);
             } else {
         ...
         login(email, password) {
             ...
             const mockUser = {
                 ...
                 email: email,
                 phone: '0912345678',
                 role: 'owner', // Default role for rich features
                 avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=256'
             };
             
             localStorage.setItem('nks_user', JSON.stringify(mockUser));
         ...
         updateProfile() {
             ...
             if (this.avatarInput) {
                 this.user.avatar = this.avatarInput;
             }
         ...
         removeFavorite(id) {
             ...
             // Dispatch event to sync detail pages
                ...
                <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm min-h-[500px]">
                    ...
                    <div x-show="activeTab === 'properties' && isLoggedIn && user && user.role === 'owner'" x-cloak class="space-y-8 animate-fade-in-up">
                        ...
                        <div class="bg-primary-extralight border border-primary/20 rounded-3xl p-6 flex flex-col sm:flex-row justify-between items-center gap-6">
                            ...
                            <button class="bg-primary hover:bg-primary-dark text-white font-bold text-xs px-6 py-3 rounded-full shadow-md shadow-primary/20 hover:shadow-lg transition-all duration-300">Đăng tin mới +</button>
                        </div>
