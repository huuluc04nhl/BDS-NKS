...
class PropertyController extends Controller
{
    ...
    /**
     * API: User Login - Xác thực hoàn toàn qua NKS API
     * Mọi tài khoản NKS hợp lệ đều có thể đăng nhập
     */
    public function apiLogin(Request $request)
    {
        try {
                ...
                try {
                    ...
                    if ($user) {
                        $user->update([
                            ...
                            'hometown' => !empty($remoteUser['hometown']) ? $remoteUser['hometown'] : (!empty($remoteUser['pob']) ? $remoteUser['pob'] : $user->hometown),
                        ...
                        $user = \App\Models\User::create([
                            ...
                            'dob' => $remoteUser['dob'] ?? '',
                            'pob' => $remoteUser['pob'] ?? '',
                            'hometown' => $remoteUser['hometown'] ?? $remoteUser['pob'] ?? '',
            ...
            Log::error('NKS Login Unexpected Error: ' . $e->getMessage() . ' | Trace: ' . $e->getTraceAsString());
            return response()->json([
                'success' => false,
            ...
            if ($remoteUser) {
                ...
                if (!$user) {
                    $user = \App\Models\User::create([
                        ...
                        'hometown' => !empty($remoteUser['hometown']) ? $remoteUser['hometown'] : (!empty($remoteUser['pob']) ? $remoteUser['pob'] : ($userData['hometown'] ?? '')),
                    ...
                    $user->update([
                        ...
                        'hometown' => !empty($remoteUser['hometown']) ? $remoteUser['hometown'] : (!empty($remoteUser['pob']) ? $remoteUser['pob'] : ($userData['hometown'] ?? $user->hometown)),
                ...
                if (!$user) {
                    $user = \App\Models\User::create([
                        ...
                        'dob' => $userData['dob'] ?? '',
                        'pob' => $userData['pob'] ?? '',
                        'hometown' => $userData['hometown'] ?? $userData['pob'] ?? '',
                    ...
                    $user->update([
                        ...
                        'dob' => $user->dob ?? $userData['dob'] ?? '',
                        'pob' => $user->pob ?? $userData['pob'] ?? '',
                        'hometown' => $user->hometown ?? $userData['hometown'] ?? $userData['pob'] ?? '',
                        ...
                        'permanent_address' => $userData['permanent_address'] ?? $user->permanent_address
                    ]);
                }
            }
    ...
    public function apiUpdateProfile(Request $request)
    {
        try {
            ...
            if ($accessToken && strpos($accessToken, 'mock_token_for_local_') === false) {
                try {
                    $params = [
                        ...
                        'pob' => $request->input('hometown') ?? $request->input('pob'),
                        ...
                        if ($remoteUser) {
                            ...
                            if ($user) {
                                ...
                                $user->update([
                                    ...
                                    'hometown' => $request->input('hometown') ?? $remoteUser['hometown'] ?? $remoteUser['pob'] ?? $user->hometown,
                            ...
                            return response()->json([
                                ...
                                'user' => [
                                    ...
                                    'hometown' => $user ? ($user->hometown ?? '') : ($remoteUser['hometown'] ?? $remoteUser['pob'] ?? ''),
            ...
            $user->update([
                ...
                'hometown' => $request->input('hometown') ?? $user->hometown,
                ...
                'permanent_address' => $request->input('permanent_address') ?? $user->permanent_address
            ]);
